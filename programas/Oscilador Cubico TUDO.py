...
for i in range(N):
    ...
    ax[i] = -(k/m)*x[i] - (3*alfa*(x[i]**2))/m   ##################mudARRRR  <--------------------

    vx[i+1] = vx[i] + ax[i]*dt

    x[i+1] = x[i] + vx[i+1]*dt

    Ep[i] = 0.5*k*(x[i]**2) + alfa*(x[i]**3)   ##############################MUDARRRR <--------------------
...
for i in range(N):
    if x[i-1] < x[i] and  x[i+1] < x[i]:
        ...
        if len(maxGraficos) == 0:
            print("O máximo do movimento é {:.2f}m.".format(abs(maxx)))
        maxGraficos.append((maxt,maxx))

## Cálculo do Minimo
## Alterar valores de primeiro if para bater certo com o gráfico de x(t) (intervalo deve ter um minimo lá dentro)
for i in range(N):
    if t[i+1] > 2 and t[i+1] < 6:
...
periodo = sum(T)/len(T)
print("O período do movimento é {:.3f}s.".format(periodo))
print("A frequência do movimento é {:.3f}Hz.".format(1/periodo))
plt.show()

## Gráfico da Energia Mecânica e Potencial
graph(x,Ep,"Energia Potencial","x(m)","Ep(J)")
plt.show()
